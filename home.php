...
if($search_monitoria) {
	$query = monitorie_query("WHERE monitorias.estudiante_id IN (SELECT estudiantes_seguidores.estudiante_id_seguido FROM estudiantes_seguidores) AND (materias.nombre LIKE '%" . $_GET['search'] . "%' OR estudiantes.nombre LIKE '%" . $_GET['search'] . "%' OR estudiantes.usuario LIKE '%" . $_GET['search'] . "%') AND monitorias.fecha_inicio > " . time());
	$query_results = $query->num_rows;
} else if($search_materia) {
	$query = subjects_query("WHERE materias.nombre like '%" . $_GET['search'] . "%' OR estudiantes.nombre LIKE '%" . $_GET['search'] . "%' OR estudiantes.usuario LIKE '%" . $_GET['search'] . "%'");
	$query_results = $query->num_rows;
